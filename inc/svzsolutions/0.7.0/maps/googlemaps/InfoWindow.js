...
/** @lends svzsolutions.maps.googlemaps.InfoWindow.prototype */
{
  ...
  init: function()
  {
      if (svzsolutions.global.mapManager.isDebugMode())
        ...
        var contentComponentElem = dojo.query('.' + this.MAIN_HOLDER_CLASSNAME, this._content);
        
        if (contentComponentElem && contentComponentElem[0])
            this.initCustomContent(contentComponentElem[0]);
        
        this.initLinks(this._content);
   ...
   * @return {Void}
   */
  initLinks: function(elem)
  {
      this._links = dojo.query('.sg-load-data', elem);
          
      // Iterate through all the list links
      for (var i = 0; i < this._links.length; i++)
      {
          dojo.connect(this._links[i], 'onclick', this, function(event)
              {
                  if (svzsolutions.global.mapManager.isDebugMode())
                      console.log('InfoWindow: Clicking link href');
                  
                  dojo.stopEvent(event);
                  
                  var a = this._findAElem(event.target);

                  var dataLoadUrl = dojo.attr(a, 'href');
                  
                  if (svzsolutions.global.mapManager.isDebugMode())
                      console.log('InfoWindow: Loading data url', dataLoadUrl, a);
   ...
   *
   * @event
   * @param {HTMLDOMElement} domElem
   * @return {Void}
   */
  initCustomContent: function(domElem)
  {
  ...
  loadDataCallback: function(data)
  {
       if (this._mainHolder)
      {
        if (data && data.content)
        {
            ...
            this.initCustomContent(domElem);
            
            this.initLinks(domElem);
        }
      }
